...
// 응답의 형태가 여러가지기 때문에 @ResponseBody를 쓰면 안됨(써버리면 무조건 데이터형태로 반환이 됨)
@Controller
public class ReturnController1 {

	// 메소드의 리턴타입에 따라 반환할 데이터의 형식이 설정됨(HTML,JSON,XML,String...)
	...
	// 리턴타입으로 String을 쓰면 파일의 경로를 직접 설정
	...
	// 리턴타입을 void로 쓰면 URL경로가 파일의 경로가 됨
